...
package func adjustBrightness(_ rgb: MLXArray, _ b: Float) -> MLXArray {
    if b == 0 { return rgb }
    return clip(rgb + b, min: 0, max: 1)
}

/// Warm/cool white balance. `t` in [-1, 1]: positive warms (more red, less blue).
package func adjustTemperature(_ rgb: MLXArray, _ t: Float) -> MLXArray {
    if t == 0 { return rgb }
...
package func posterize(_ rgb: MLXArray, levels: Int) -> MLXArray {
...
package func threshold(_ rgb: MLXArray, _ t: Float) -> MLXArray {
    let r = rgb[0..., 0..., 0]
    let g = rgb[0..., 0..., 1]
    let b = rgb[0..., 0..., 2]
...
package func vignette(_ rgb: MLXArray, amount: Float) -> MLXArray {
...
package func autoContrast(_ rgb: MLXArray) -> MLXArray {
    let mn = MLX.min(rgb)
    let mx = MLX.max(rgb)
